...
type ElectronAPI = {
	...
	conflict: {
		...
		getDiff: (filePath: string) => Promise<Record<string, unknown>>;
		resolve: (
			fileId: string,
...
const electronAPI: ElectronAPI = {
	config: {
		...
		save: (configPath, config) =>
			ipcRenderer.invoke("config:save", configPath, config),
		validate: (config) => ipcRenderer.invoke("config:validate", config),
		setDefaultPath: (configPath) =>
			ipcRenderer.invoke("config:setDefaultPath", configPath),
	...
	preset: {
		...
		delete: (presetId) => ipcRenderer.invoke("preset:delete", presetId),
		load: (presetId) => ipcRenderer.invoke("preset:load", presetId),
		getDefault: () => ipcRenderer.invoke("preset:getDefault"),
		setDefault: (presetId) => ipcRenderer.invoke("preset:setDefault", presetId),
		rename: (presetId, newName) =>
			ipcRenderer.invoke("preset:rename", presetId, newName),
		updateDescription: (presetId, description) =>
			ipcRenderer.invoke("preset:updateDescription", presetId, description),
		duplicate: (presetId, newName) =>
			ipcRenderer.invoke("preset:duplicate", presetId, newName),
	},
	git: {
	...
	conflict: {
		...
		resolve: (fileId, strategy) =>
			ipcRenderer.invoke("conflict:resolve", fileId, strategy),
	},
	report: {
